...
Move-Item -Path ".\discord_game_sdk\cpp" -Destination ".\src\discord" -Force
if ( $? ) {
  Write-Host ":: " -ForegroundColor Green -NoNewline
...
if ( $DEBUG ) {
  patch ./src/discord/types.h ./patches/src/discord/types.h.patch
  if ( $? ) {
    Write-Host ":: " -ForegroundColor Green -NoNewline
    Write-Host "Applied patch to types.h"
  } else {
    Write-Host ":: " -ForegroundColor Red -NoNewline
    Write-Host "Could not apply patch for file src/discord/types.h!"
  }
} else {
  patch ./src/discord/types.h ./patches/src/discord/types.h.patch | Out-Null
  if ( $? ) {
    Write-Host ":: " -ForegroundColor Green -NoNewline
    Write-Host "Applied patch to types.h"
  } else {
